...
* IA Generativa / LLM: Google Gemini API (`google-genai` SDK) — Modelo `gemini-3.5-flash` (configurável via `GEMINI_MODEL`)
* Interface Web: Streamlit (com estilização customizada via CSS Injection)
* Prompt Engineering: saída JSON nativa via `response_mime_type="application/json"`
* Segurança & Gestão de Variáveis: `python-dotenv`, escape de HTML (`html.escape`) em todo dado renderizado
...
├── gemini_processor.py     # Lógica compartilhada de chamada à API do Gemini
...
A variável `GEMINI_MODEL` é opcional — se omitida, o projeto usa `gemini-3.5-flash` por padrão.
...
* Todo dado vindo do modelo é sanitizado com `html.escape()` antes de ser renderizado na interface, prevenindo XSS.
* A chave de API nunca é commitada — `.env` está listado no `.gitignore`.
* Dependências auditadas com [`pip-audit`](https://pypi.org/project/pip-audit/) e código analisado com [`bandit`](https://pypi.org/project/bandit/).